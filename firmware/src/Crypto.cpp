...
memcpy(currentKey, key, 16);
mbedtls_aes_setkey_enc(&aesCtx, key, 128);
    keySet = true;
}

void Crypto::setKey(const uint8_t* newKey){
memcpy(currentKey, newKey, 16);
mbedtls_aes_setkey_enc(&aesCtx, newKey, 128);
    keySet = true;
}

void Crypto::getKey(uint8_t* outKey) const {
memcpy(outKey, currentKey, 16);
}
